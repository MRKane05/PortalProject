...
namespace Portals {
    ...
    public class PortalRenderer : RenderedBehaviour {
        ...
        private void Awake() {
            ...
            {
                frontFaceShader = Shader.Find("Portal/BasicPortal");
            }
            if (backFaceShader == null)
            {
                backFaceShader = Shader.Find("Portal/BasicPortalBackface");
            }
            if (invisibleShader == null)
            {
                invisibleShader = Shader.Find("Portal/Invisible");
            }
        ...
        private Camera CreateTemporaryCamera(Camera currentCamera) {
            Camera camera;
            _cameraMap.TryGetValue(currentCamera, out camera);
            if (!camera) {
